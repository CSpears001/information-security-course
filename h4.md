...
<img width="1472" height="1316" alt="Näyttökuva 2025-09-15 184201" src="https://github.com/user-attachments/assets/a4da5807-9265-409a-a3c7-b681020fd378" />
...
- So first I wanted to try a pretty common way to see if the application is vulnerable so use a single quote so '
...
- We want the application to show all the products so we give it the payload ' or 1=1 --' which is a conditional statement. We need to bypass the AND released = 1 condition. That is done with the ' or 1=1 --' injection. 
...
- -- starts a comment in the SQL which makes it so that the original AND released = 1 is ignored
- OR 1=1 is always going to be 2 so all the rows will be shown regardless of the category or being released or unreleased.
...
- Portswigger Labs: SQK injection vulnerability in WHERE clause allowing retrieval of hidden data https://portswigger.net/web-security/sql-injection/lab-retrieve-hidden-data
